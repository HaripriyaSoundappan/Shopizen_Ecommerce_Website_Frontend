...
const OfferPage = () => {
  ...
  const offerFilteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      switch (offer.filterType) {
        ...
        case "subCategory":
          return (
            p.subCategory?.toLowerCase() === offer.filterValue.toLowerCase()
          );
        default:
          return true;
      }
  ...
  if (!offer)
    return <div className="text-center mt-5">Offer not found or expired.</div>;

  const totalTime = offer?.expiry
    ? Math.floor((new Date(offer.expiry) - new Date()) / 1000) + timeLeft
    : 1;

  return (
    <>
      {/* Countdown Timer with Animated Progress */}
      {offer?.expiryDate && <OfferTimer expiryDate={offer.expiryDate} />}
            ...
            {currentProducts.map((p) => (
              <div className="col-lg-4 col-md-6 col-12 mb-4" key={p.id}>
                <ProductCard
                  product={{
                    ...p,
                    price: (p.price - (p.price * offer.discount) / 100).toFixed(
                      2,
                    ),
                    discount: offer.discount,
                  }}
...
function OfferTimer({ expiryDate }) {
  ...
  React.useEffect(() => {
    ...
    const expiry = new Date(
      isDateOnly ? `${expiryDate}T23:59:59` : expiryDate,
    ).getTime();
    const now = Date.now();
